...
void ArtificalHorizon::CreateAH(ScaneLineRenderer & renderer) {
	...
	sky.SetValue(0, 120, 319, 120, centerx, 0, 1, 0x00ff);
	// Ground
	renderer.AddDrawableObj(&ground);
	ground.SetValue(0, 120, 319, 120, centerx, 239, 1, 0xf00f);
	...
	// Center part
	rightRect = FilledRectWithFrame(0, 110, 30, 20, 1, color565(100, 100, 100), 0xffff, true);
...
void ArtificalHorizon::SetValues(ScaneLineRenderer & renderer, float pitch, float roll) 
{
	...
	text.SetValue(5, 115, 2, buffer1, 1, 0xffff);
	...
	// sky and ground
	t1.x = -400; t1.y = pitch *deg2Pix;
	t2.x = 400;  t2.y = pitch *deg2Pix;
	t3.x = 0;    t3.y = -2200 - pitch *deg2Pix; // sky

	Point t4;
	t4.x = 0; t4.y = 2200 + pitch *deg2Pix;

	Rotate(t1, rotateV);    Rotate(t2, rotateV);
	Rotate(t3, rotateV);    Rotate(t4, rotateV);

	sky.SetValue(t1.x + centerx, 120 + t1.y,
		t2.x + centerx, 120 + t2.y,
		t3.x + centerx, 120 + t3.y, 1, color565(50, 50, 200));

	ground.SetValue(t1.x + centerx, 120 + t1.y,
		t2.x + centerx, 120 + t2.y,
		t4.x + centerx, 120 + t4.y, 1, 0x0078);

	// HorizontalLine
	for (int i = 0; i < 2; i++) {

		Point leftP(-300, pitch *deg2Pix - 1 + i);
		Point rightP(300, pitch *deg2Pix - 1 + i);
	...
	for (int i = -30; i <= 30; i += 5, o++) {
		...
		int pitch2 = pitch;
		if (i == 10) {
			itoa(((pitch2 / 10) * 10 - 10), buffer, 10);
		...
		if (i == -10) {
			itoa(((pitch2 / 10) * 10 + 10), buffer2, 10);
